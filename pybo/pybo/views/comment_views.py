...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect('{}#qcomment_{}'.format(url_for('question.detail', question_id=question_id), comment.id))
...
def modify_question(comment_id):
    ...
    if request.method == 'POST' :
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            return redirect('{}#qcomment_{}'.format(url_for('question.detail', question_id=comment.question.id), comment.id))
    else:
...
def create_answer(answer_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.add(comment)
        db.session.commit()
        return redirect('{}#acomment_{}'.format(url_for('question.detail', question_id=answer.question.id), comment.id))
    return render_template('comment/comment_form.html', form=form)
...
def modify_answer(comment_id):
    ...
    if request.method == 'POST' :
        ...
        if form.validate_on_submit():
            ...
            comment.modify_date = datetime.now()
            db.session.commit()
            return redirect('{}#acomment_{}'.format(url_for('question.detail', question_id=comment.answer.question.id), comment.id))
    else:
        form = CommentForm(obj=comment)
    return render_template('comment/comment_form.html',form=form)
